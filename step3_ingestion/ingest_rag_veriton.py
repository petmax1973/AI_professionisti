...
def populate_vector_db():
    print("Initializing the local Embedding model for Acer Veriton VN100 GB10 Blackwell...")
    
    # Initialize the embeddings model with CUDA and massive batch size for GB10 GPU
    try:
        embeddings = HuggingFaceEmbeddings(
            model_name=EMBEDDING_MODEL_NAME,
            model_kwargs={'device': 'cuda'},
            encode_kwargs={'batch_size': 256} # Increased batch size for Blackwell
        )
    except Exception as e:
        print(f"Warning: Initialization on CUDA failed. Fallback to CPU. Error: {e}")
        embeddings = HuggingFaceEmbeddings(
            model_name=EMBEDDING_MODEL_NAME,
    ...
    # Enormous batch size taking advantage of the 128GB LPDDR5x / Blackwell Superchip
    batch_size = 2000
    db = None
    total_processed = 0
