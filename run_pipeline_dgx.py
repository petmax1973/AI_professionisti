...
def branch_normattiva(results):
    """Gestisce il ramo A: Download e Preprocessing per Normattiva (Acer Veriton)"""
    log = "normattiva.log"
    # Fase 1
    if not run_command("cd step1_download_laws && source venv/bin/activate && python3 -u export_laws_2.py", "Normattiva - Download (Fase 1)", log):
        ...
        return
    # Fase 2 (Inizia immediatamente non appena finisce la Fase 1) - Versione Acer Veriton
    if not run_command("cd step2_preprocessing && source venv/bin/activate && python3 -u preprocess_normativa_acer.py", "Normattiva - Preprocessing Acer (Fase 2)", log):
        results['normattiva'] = False
        return
...
def branch_agenzia(results):
    """Gestisce il ramo B: Scraping e Preprocessing per l'Agenzia delle Entrate (Acer Veriton)"""
    log = "agenzia.log"
    # Fase 1
    if not run_command("cd step1_download_laws && source venv/bin/activate && python3 -u scraping_data.py", "Agenzia Entrate - Scraping (Fase 1)", log):
        ...
        return
    # Fase 2 (Inizia immediatamente non appena finisce la Fase 1) - Versione Acer Veriton
    if not run_command("cd step2_preprocessing && source venv/bin/activate && python3 -u preprocess_agenzia_acer.py", "Agenzia Entrate - Preprocessing Acer (Fase 2)", log):
        results['agenzia'] = False
        return
...
def main():
    ...
    print("Avvio della pipeline OTTIMIZZATA per Acer Veriton VN100 (Grace-Blackwell GB10)...")
    ...
    print(f"\n{'='*50}\nFasi 1 e 2 completate per tutti i rami. Avvio Fase 3 (Ingestion su GPU Blackwell)...\n{'='*50}")
    
    # ---------------------------------------------------------
    # STEP 3: Ingestion (Sequenziale, attende tutti) - Versione Acer Veriton
    # ---------------------------------------------------------
    step3_command = "cd step3_ingestion && source venv/bin/activate && python3 -u ingest_rag_veriton.py"
    if not run_command(step3_command, "Ingestion Database Vettoriale ChromaDB Veriton (Fase 3)", "ingestion.log"):
        print("\nInterruzione pipeline a causa di un errore nello Step 3.")
        sys.exit(1)
        
    elapsed_time = time.time() - start_time
    print(f"\n{'='*50}\nPIPELINE DGX COMPLETATA CON SUCCESSO!\nTempo totale: {elapsed_time:.2f} secondi.\n{'='*50}")
