...
def build_rag_dataset():
    print("=" * 70)
    print("Inizio elaborazione parallela dataset Normattiva (Acer Veriton High-Perf)")
    print("=" * 70)
    ...
    print(f"Trovati {len(all_files)} file. Avvio del pool di processi (fino a 20 core)...")
    
    # Multiprocessing: esegue l'analisi e il partizionamento strutturale in parallelo
    # Usando executor.imap o executor.map possiamo mantenere un flusso in streaming per il salvataggio
    with open(output_path, 'w', encoding='utf-8') as out_file:
        with concurrent.futures.ProcessPoolExecutor() as executor:
            # imap itera sui risultati man mano che i processi li completano (nell'ordine in cui terminano o in ordine, dipende dalla funzione)
            for i, articles in enumerate(executor.map(process_json_file, all_files)):
                if (i + 1) % 100 == 0 or (i + 1) == len(all_files):
                    print(f"Progresso: completati {i+1}/{len(all_files)} documenti...")
    ...
    print("\n" + "=" * 70)
    print(f"Elaborazione parallela completata in tempi minimi!")
    print(f"File letti:   {len(all_files)}")
    print(f"Chunk totali: {total_articles}")
