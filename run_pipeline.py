...
def run_command(cmd, step_description):
...
def branch_normattiva(results):
    ...
    # Fase 1
    if not run_command("cd step1_download_laws && source venv/bin/activate && python3 export_laws_2.py", "Normattiva - Download (Fase 1)"):
        results['normattiva'] = False
        return
    # Fase 2 (Inizia immediatamente non appena finisce la Fase 1)
    if not run_command("cd step2_preprocessing && python3 preprocess_rag.py", "Normattiva - Preprocessing (Fase 2)"):
        results['normattiva'] = False
        return
...
def branch_agenzia(results):
    ...
    # Fase 1
    if not run_command("cd step1_download_laws && source venv/bin/activate && python3 scraping_data.py", "Agenzia Entrate - Scraping (Fase 1)"):
        results['agenzia'] = False
        return
    # Fase 2 (Inizia immediatamente non appena finisce la Fase 1)
    if not run_command("cd step2_preprocessing && source venv/bin/activate && python3 preprocess_agenzia.py", "Agenzia Entrate - Preprocessing (Fase 2)"):
        results['agenzia'] = False
        return
...
def main():
    ...
    # ---------------------------------------------------------
    step3_command = "cd step3_ingestion && source venv/bin/activate && python3 ingest_rag.py"
    if not run_command(step3_command, "Ingestion Database Vettoriale ChromaDB (Fase 3)"):
